...
type
  TParse_ToNotFound = (tfError, tfEmpty, tfLast);
...
resourcestring
  rsErrToNotFound  = 'Не удалось найти пару идентификаторов "%1:s" - "%2:s" в тексте'#13#13'[%0:s]!';
...
begin
  ...
  iPosFrom := Pos(UpperCase(sFrom), UpperCase(sText));
  ...
  begin
    ...
    else iLenCopy := Pos(UpperCase(sTo), UpperCase(RightStr(sText, Length(sText) - iPosFrom + 1))) - 1;

    if iLenCopy < 0 then
    begin
      case aToMode of
        tfError:
          raise Exception.CreateFmt(rsErrToNotFound, [sText, sFrom, sTo]);
        tfLast:
...
begin
  ...
  begin
    ...
    begin
      ...
      iLenCopy := Pos(UpperCase(sTo), UpperCase(RightStr(sText, Length(sText) - iPosFrom + 1))) - 1;
...
begin
  ...
  begin
    ...
    iPosNext := PosEx(UpperCase(sFrom), UpperCase(sText), iPosFrom + 1);
    if iPosNext = 0 then
...
begin
  ...
  if AnsiContainsText(sCharset, 'utf-8') then
    Result := Utf8ToAnsi(sText);
end;
