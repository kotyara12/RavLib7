...
uses
  RxStrUtils, SysUtils, WinSock, RDialogs;
...
function GetIpSubnet(const Net, Mask: RIpAddr; out FirstIp, LastIp: RIpAddr): Cardinal;
begin
  Result := not Mask.Socket;
  FirstIp.Socket := Net.Socket and Mask.Socket;
  LastIp.Socket := FirstIp.Socket + Result;
end;
...
begin
  ...
  try
    HE := GetHostByName(PChar(NetName));
    if HE <> nil then Result := Inet_ntoa(PInAddr(HE.h_addr_list^)^);
  finally
    WSACleanup;
  end;
end;
...
begin
  Result := 0;
  if MaskLen > 32 then IntLen := 32 else IntLen := MaskLen;
