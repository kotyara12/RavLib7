(*
Here is some source I wrote for a threaded pipe server component, as well as a pipe client component. 
(register the unit and the components will show up in the Win32 tab). 
The server component is what your after, and it allows for up to 100 concurrent clients 
(can be changed by modifying the MAX_THREADS constant). 

The following events are available from the server component:

OnPipeConnect    =  procedure(Sender: TObject; Pipe: HPIPE) of object;
OnPipeDisconnect =  procedure(Sender: TObject; Pipe: HPIPE) of object;
OnPipeMessage    =  procedure(Sender: TObject; Pipe: HPIPE; Stream: TStream) of object;
OnPipeSent       =  procedure(Sender: TObject; Pipe: HPIPE; Size: DWORD) of object;
OnPipeError      =  procedure(Sender: TObject; Pipe: HPIPE; PipeContext: TPipeContext; ErrorCode: Integer) of object;

And should allow you to control most aspects of the pipe handling. 

The OnPipeMessage 
-----------------
will get fired when a client has written data 
(and all the data has been retrieved) to the server pipe, and the data can be accessed 
via the stream parameter that is passed over. 
To write data back to the client pipe, you can call the Write method which is declared as:

// Similar to a TStream's write method
