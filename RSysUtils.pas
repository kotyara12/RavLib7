...
function WinExec32(WorkDir, CmdLine: string; const ShowWindow: Integer;
  ...
  const MaxWaitTime: Cardinal): DWord;
...
resourcestring
  SWaitExitBreak       = 'Ожидание завершения процесса (файла) "%s" прервано!';
  SWaitExitTimeout     = 'Ожидание завершения процесса (файла) "%s" прервано в связи с превышением времени ожидания!';
...
begin
  ...
  with MySLink do
  begin
    SetPath(PAnsiChar(CmdLine));
    SetArguments(PAnsiChar(Args));
    ...
    begin
      try
        ...
        begin
          ...
          if ExitWait then
            raise ESystemError.CreateFmt(SWaitExitBreak, [ExtractFileName(CmdLine)])
          else Sleep(100);
          ...
          begin
            ...
            begin
              ...
              raise ESystemError.CreateFmt(SWaitExitTimeout, [ExtractFileName(CmdLine)]);
            end;
          end;
        end;
        Result := ExitCode;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        else raise ESystemError.CreateFmt(SErrFindProcedure,
          [WinExecProcName, AdvApiDllName]);
      end
      else raise ESystemError.CreateFmt(SErrLoadLibrary, [GetSystemError(False)]);
    finally
      if Assigned(WinExecProc) then
        @WinExecProc := nil;
      if AdvApiDll <> INVALID_HANDLE_VALUE then
    ...
    begin
      ...
      begin
        ...
        if Assigned(CbWaitExitChk) then
          CbWaitExitChk(ExitWait);
        if ExitWait then
          raise ESystemError.CreateFmt(SWaitExitBreak, [ExtractFileName(FileName)])
        else Sleep(100);
        if MaxWaitTime > 0 then
        begin
          if (GetTickCount - WaitStart) > (MaxWaitTime * 1000) then
          begin
            ...
            raise ESystemError.CreateFmt(SWaitExitTimeout, [ExtractFileName(FileName)]);
          end;
        end;
      end;
...
begin
  ...
  NetResource.lpRemoteName := PAnsiChar(RemoteName);
  NetResource.lpProvider := '';
  if Trim(UserName) = EmptyStr then lpUserName := nil
  else lpUserName := PAnsiChar(Trim(UserName));
  if Trim(Password) = EmptyStr then lpPassword := nil
  else lpPassword := PAnsiChar(Trim(Password));
  dwFlags := 0;
...
begin
  dwFlags := 0;
  if SaveProfile then dwFlags := CONNECT_UPDATE_PROFILE;
  Result := WNetCancelConnection2(PAnsiChar(LocalName), dwFlags, ForceMode);
end;
...
procedure OpenUrl(const sUrl: string);
begin
  if AnsiStartsText('http://', sUrl) or AnsiStartsText('https://', sUrl)
  or AnsiStartsText('ftp://', sUrl) or AnsiStartsText('ftps://', sUrl)
  then ShellExecute(0, nil, PAnsiChar(sUrl), nil, nil, 1)
  else ShellExecute(0, nil, PAnsiChar('http://' + sUrl), nil, nil, 1);
end;

{ == Создать электронное письмо программой по умолчанию ======================== }
procedure CreateEMail(const sHeader: string);
begin
  if AnsiStartsText('mailto:', sHeader)
  then ShellExecute(0, nil, PAnsiChar(sHeader), nil, nil, 1)
  else ShellExecute(0, nil, PAnsiChar('mailto:' + sHeader), nil, nil, 1);
end;
...
function CheckEMail(const sMail: string): Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        if not (sUser[i] in ['a'..'z', 'A'..'Z', '0'..'9', '_', '-', '.']) then
        begin
          Result := False;
          Break;
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        if not (sDomain[i] in ['a'..'z', 'A'..'Z', '0'..'9', '_', '-', '.']) then
        begin
          Result := False;
          Break;
        end;
    ...
    begin
      ...
      begin
        if sDomain[i] = '.' then Break
          ...
          if not (sDomain[i] in ['a'..'z', 'A'..'Z']) then
          begin
            Result := False;
            Break;
          end;
