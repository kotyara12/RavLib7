unit TmplDbColorDialog;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, TmplDbDialog, DB, StdCtrls, Buttons, ExtCtrls;

type
  TDbColorDialogTemplate = class(TDbDialogTemplate)
...
procedure TDbColorDialogTemplate.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TDbColorDialogTemplate.InitFormVariables;
begin
  inherited;
  FCellColorField := nil;
  FFontColorField := nil;
...
procedure TDbColorDialogTemplate.InitComponents(const EditMode: Boolean);
begin
  try
    inherited;
  finally
...
procedure TDbColorDialogTemplate.UpdateControls(Sender: TObject);
begin
  inherited;
...
procedure TDbColorDialogTemplate.DecodeColors(Sender: TObject);
begin
  FCellColor := clWindow;
  FFontColor := clWindowText;
  FFontStyle := 0;
...
{ == Смена цвета фона ========================================================== }
procedure TDbColorDialogTemplate.PutCellColor;
begin
...
procedure TDbColorDialogTemplate.SetCellColor(Sender: TObject);
begin
...
procedure TDbColorDialogTemplate.CellColorBtnClick(Sender: TObject);
begin
...
{ == Смена цвета текста ======================================================== }
procedure TDbColorDialogTemplate.PutFontColor;
begin
...
procedure TDbColorDialogTemplate.SetFontColor(Sender: TObject);
begin
...
procedure TDbColorDialogTemplate.FontColorBtnClick(Sender: TObject);
begin
...
{ == Смена стиля шрифта ======================================================== }
procedure TDbColorDialogTemplate.PutFontStyle;
begin
  TestText.Font.Style := [];
  if FontStyleCheckBox.Checked then
  begin
...
procedure TDbColorDialogTemplate.SetFontStyle(Sender: TObject);
begin
...
procedure TDbColorDialogTemplate.SetFontStyleProp(Sender: TObject);
begin
  FFontStyle := 0;
