{* DelLoc. LcUnit.pas. Serge Gavrilov ( C ) 2005-2009 *********************** *}
...

{
  ...
  All right reserved. Serge Gavrilov (C) 2005-2009.
...
{* ************************************************************************** *}
  ...
  public
    ...
    function GetResString( pResStrRec : PResStringRec ) : string;
    function TranslateComponent(
      oComponent : TComponent;
      iLCID : LCID ) : boolean;
...
begin
  ...
  result := LoadLib( szFileName, pOpts^.iLCID, pOpts^.pCallBack,
    pOpts^.pOnGetResStr, pOpts^.pUd );
end;
{* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *}
...
var
  iDefLen : integer;
  wBuffer : array [ 0..1023 ] of WideChar;
  iPrefix : integer;
  iLCID : integer;
  iCPage : UINT;
  iLangIndex : integer;
...
begin
  ...
  then  begin
        GetDefault;
        if iDefLen > 0
        then  iPrefix := pOnGetResStr( PWideChar( @wBuffer ), iLCID,
                iChangedCPage, pUd );
        end;
        ...
        then  begin
              ...
              then  begin
                    ...
                    then  begin
                          ...
                          result := WideStringToString( wString, iCPage );
                          exit;
                          end;
                    end;
              end;
  ...
  if iDefLen > 0
  then  begin
        result := WideStringToString( PWideChar( @wBuffer ), iCPage );
...
begin
  ...
  then  result := PChar( ResStringRec.Identifier )
...
begin
  _oLcf.Free;
  _oLcf := TLcf.Create;
  result := _oLcf.LoadLibEx( PChar( sFileName ), pOpts );
  if result < 1
  then  begin
        _oLcf.Free; _oLcf := nil;
        exit;
...
{$else}
                      if pProp^.PropType^.Kind in [ tkString, tkLString ]
                      then  SetWideStrProp( oObject, pProp^.Name,
                        WideStringToString( wTranslation, iCPage ) )
                      else  SetWideStrProp( oObject, pProp,
                        wTranslation );
{$endif}
                      end;
                end;
