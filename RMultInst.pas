...
const
  ...
  MI_FAIL_SUBCLASS_S     = 'Ошибка инициализации процедуры обработки сообщений Windows!';
  MI_FAIL_CREATE_MUTEX_S = 'Ошибка создания мьютекса "%s"!';

function GetMIError: Integer;

implementation

uses Forms, Windows, SysUtils, RDialogs;

var
  UniqueAppStr: string = 'RAVSOFT.RMULTINST.';
  RegMsgId: Integer;
...
begin
  ...
  begin
    ...
    {$ELSE}
    ErrorBox(Format(MI_FAIL_CREATE_MUTEX_S, [UniqueAppStr]));
    {$ENDIF}
  end;
end;
...
begin
  SubClassApplication;
  if OpenMutex(MUTEX_ALL_ACCESS, False, PChar(UniqueAppStr)) = 0
  then DoFirstInstance
  else BroadcastFocusMessage;
end;

initialization
  UniqueAppStr := UniqueAppStr + ExtractFileName(Application.ExeName);
  RegMsgId := RegisterWindowMessage(PChar(UniqueAppStr));
  InitInstance;
